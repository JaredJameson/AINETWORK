...
export default function NewArticlePage() {
  ...
  useEffect(() => {
    fetch('/api/categories?type=article').then(r => r.json()).then(setCategories).catch(() => {});
    fetch('/api/articles?type=pillar').then(r => r.json()).then(setPillars).catch(() => {});
  }, []);
      ...
      <form onSubmit={e => handleSubmit(e, form.published)}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '16px', marginBottom: '16px', alignItems: 'start' }}>
          ...
          <div style={{ minWidth: '120px' }}>
            ...
            <input style={inputStyle} value={form.readingTime} onChange={e => set('readingTime', e.target.value)} placeholder="10 min" />
          </div>
        </div>
